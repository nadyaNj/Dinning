...
<form id="createSurpleMenu" class="clearfix" name="createSurpleMenu" method="post" action="<%=request.getContextPath()%>/actionServlet">
    	...
    	<tr>
			...
			<c:forEach items="${addedProdList}" var="addedProd">
				<c:forEach items="${menu}" var="menuItems">
					<c:if test="${addedProd.id == menuItems.surplusProductId}">
						...
						<tr id='<c:out value="${addedProd.id}"/>'>
							...
							<td>
								<input name='<c:out value="${addedProd.id}"/>' value='<c:out value="${menuItems.surplusProductCount}" />' class="input-text-short" type="text" size="10" maxlength="8" />
								<%-- <c:out value="${menuItems.productCount}" /> --%>
							</td>
						<tr>
					</c:if>
				</c:forEach>
	...
	<div class="button-line fright">
		<a href="#" class="button"  onclick='save();'>Edit</a>		
	</div>
</form> 
</div>
</body>
